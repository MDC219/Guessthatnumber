...
// Starting alert message alert message starts with ()
...
while (restartGame){
  ...
  while (!rangeNum || rangeNum < 1) {   //!range num is a parseInt
    rangeNum = prompt(enterNumText)
    rangeNum = parseInt(rangeNum);
  }

  // Creates the random number using the range number entered by the user
  randomNum = Math.floor(Math.random() * rangeNum) + 1; //.floor chops off decimal to leave whole number
  ...
  while (true){
    ...
    while (!guess || guess < 1 || guess > rangeNum){  //!guess is a number greater than zero and less than range num
      guess = parseInt(prompt(`Please enter a number from 1 to ${rangeNum}`));
    }

    // Removes an attempt
    attempts--;  //attempts-- decreases the number of attempts
